...
<div class="space-y-6">
    <div class="bg-white rounded-lg shadow p-4">
        <div class="flex flex-wrap justify-between items-center gap-4">
            <a href="{{ route('admin.users.create') }}" class="bg-mso-gold text-mso-blue px-4 py-2 rounded-lg hover:bg-mso-blue hover:text-white transition-colors flex items-center gap-2 shadow-sm">
            ...
            <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-wrap gap-2" autocomplete="off">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Buscar usuario..." autocomplete="off" class="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-mso-gold focus:border-transparent outline-none w-full sm:w-auto">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($users ?? [] as $userItem)
                        <tr class="hover:bg-gray-50 transition-colors {{ $userItem->is_active ? '' : 'bg-gray-50 opacity-75' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img class="h-10 w-10 rounded-full object-cover" src="{{ $userItem->profile_photo_url }}" alt="{{ $userItem->full_name }}">
                                    </div>
                                    <div class="ml-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $userItem->is_active ? 'text-gray-500' : 'text-gray-400' }}">{{ $userItem->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $userItem->is_active ? 'text-gray-500' : 'text-gray-400' }}">{{ $userItem->phone ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick="openModal('{{ route('admin.users.modal-show', $userItem) }}')" class="text-gray-400 hover:text-mso-blue transition-colors p-1" title="Ver detalle">
                                        <i class="ph ph-eye text-lg"></i>
                                    </button>
                                    @if(!$userItem->hasRole('Cliente'))
                                    <button onclick="openModal('{{ route('admin.users.modal-edit', $userItem) }}')" class="text-gray-400 hover:text-yellow-500 transition-colors p-1" title="Editar">
                                        <i class="ph ph-pencil text-lg"></i>
                                    </button>
                                    @endif
                                    @if(auth()->id() != $userItem->id)
                                        <button onclick="openModal('{{ route('admin.users.modal-delete', $userItem) }}')" class="text-gray-400 hover:text-red-600 transition-colors p-1" title="Eliminar">
