...
{{--  BOTÓN DE CONFIGURACIÓN - AQUÍ ES DONDE VA --}}
@if($isAsesor || $isAdmin || in_array('Super Admin', $userRoles))
...
@if($isAsesor)
<div class="space-y-6">
    {{-- SECCIÓN SUPERIOR: CALENDARIO + FILTROS --}}
                    ...
                    <tr class="border-b hover:bg-slate-50">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex justify-center gap-2">
                                <button onclick="window.showPropertyPreview({{ $appointment->id }})" class="text-mso-blue" title="Ver propiedad"><i class="ph-bold ph-eye text-lg"></i></button>
                                <button onclick="window.openAdminActionModal({{ $appointment->id }})" class="text-mso-blue" title="Gestionar"><i class="ph-bold ph-gear text-lg"></i></button>
                            </div>
                        </td>
                    </tr>
                    @empty
...
window.showPropertyPreview = function(appointmentId) {
    ...
    // URL CORRECTA usando ID en lugar de slug
