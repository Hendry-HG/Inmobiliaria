...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    {{-- ============================================= --}}
    {{-- FORMULARIO - Solo con permiso de editar       --}}
    {{-- ============================================= --}}
    @can('editar servicios')
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50">
                ...
                <p class="text-sm text-slate-500 mt-1">Actualiza la información del servicio</p>
            </div>

            <form action="{{ route('admin.servicios.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea name="description" rows="4"
                              ...
                              placeholder="Describe el servicio que ofreces...">{{ old('description', $service->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="color" name="color" value="{{ old('color', $service->color ?? '#c5a059') }}"
                               class="w-12 h-12 rounded-lg border border-slate-300 cursor-pointer @error('color') border-red-500 @enderror">
                        <input type="text" name="color_text" value="{{ old('color', $service->color ?? '#c5a059') }}"
                ...
                <div>
                    ...
                    <input type="url" name="external_url" value="{{ old('external_url', $service->external_url) }}"
                ...
                <div>
                    ...
                    @if($service->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->title }}" class="w-32 h-32 object-cover rounded-lg border border-slate-200">
                    ...
                    <p class="text-xs text-slate-400 mt-1">Formatos: JPG, PNG, WebP. Máximo 2MB. Dejar vacío para mantener la imagen actual.</p>
                    @error('image')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select name="is_featured" class="w-full border border-slate-300 rounded-lg p-2.5 focus:ring-2 focus:ring-mso-gold focus:border-mso-gold">
                            ...
                            <option value="1" {{ old('is_featured', $service->is_featured) == '1' ? 'selected' : '' }}>Sí</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-4 pt-4 border-t">
                    ...
                    <button type="submit" class="bg-mso-blue text-white px-8 py-2.5 rounded-lg hover:bg-slate-800 font-bold shadow-lg transition-all hover:-translate-y-0.5">
                        <i class="ph ph-floppy-disk mr-1"></i> Actualizar Servicio
                    </button>
                </div>
            </form>
        </div>
    @else
        <div class="bg-red-50 border border-red-200 text-red-700 p-6 rounded-lg text-center">
            ...
            <p class="text-sm">No tienes permisos para editar servicios.</p>
            <a href="{{ route('admin.servicios.index') }}" class="text-mso-blue hover:underline mt-2 inline-block">
                Volver al listado
            </a>
        </div>
    @endcan
